...
async function createTask(title, content, isCompleted, userId) {
    //console.log(`${title},${content},,${isCompleted},${userId}`)
    try {
        if (!title || !content)
...
async function createUser(name, email, password, age, gender,isAdmin) {
    try {
        if (!name || !email || !password || !age || !gender)
            throw new Error('Missing required parametres')
        const database = client.db('myapp')
        const collection = database.collection('users')
        const result = await collection.insertOne({ name, email, password, age, gender ,isAdmin })
...
async function updateUser(id,name,email,age,gender,isAdmin){
    try{
        console.log(id,name,email,age,country,gender,isAdmin)
        const database=client.db('myapp')
        const collection=database.collection('users')
        const Id= new ObjectId(id)
        const updateItem=await collection.updateOne({_id:Id},{$set:{name:name,email:email,age:age,gender:gender,isAdmin:isAdmin}})
        return updateItem.modifiedCount > 0;
    }
    catch(err){
        console.error('Error: ',err)
